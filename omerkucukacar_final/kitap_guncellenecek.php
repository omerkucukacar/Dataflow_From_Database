<?php
    ...
    $guncellenecek = mysqli_real_escape_string($baglanti, $_GET["kkn"]);
    $sorgu = mysqli_query($baglanti, "SELECT * FROM kitapveri WHERE kkn = $guncellenecek");
    $satir = mysqli_fetch_assoc($sorgu);
    $sorgu2 = mysqli_query($baglanti, "SELECT *, CONCAT(ya,' ', ys) AS AdSoyad FROM yazarveri");
...
Yazar Adı:    <select style="position:relative;left:95px" name="ki" id="ki">
    ...
    <option value="<?php echo $satir2['yazarkayitno'];  ?>"><?php echo $satir2["AdSoyad"]; ?></option>
  <?php  }?>
</select><br><br>
    ...
    <option value="<?php echo $satir3['yykn'];  ?>"><?php echo $satir3["yya"]; ?></option>
  <?php  }?>
</select><br><br>
