<?php
    ...
    $ya = mysqli_real_escape_string($baglanti, $_POST['ya']);
    $ys = mysqli_real_escape_string($baglanti, $_POST['ys']);
    $yu = mysqli_real_escape_string($baglanti, $_POST['yu']);
    $sorgu = "INSERT INTO yazarveri (ya, ys, yu)
            VALUES ('$ya', '$ys', '$yu')";
    ...
    $sorgu2 = mysqli_query($baglanti, "SELECT * FROM yazarveri ORDER BY yazarkayitno DESC");
...
<!-- KODLAR -->
<h1>Yazar İşlem Sonucu</h1>
<p><?php echo $islemSonuc; ?></p>
<p><a href="yazar_ekle.php" style="text-align:center;background-color:#212121; color: white">Yeni Yazar Ekle</a></p>
<?php
...
while($satir = mysqli_fetch_assoc($sorgu2)){
    ...
    echo("<td>".$satir["ykt"]."</td>");
    echo("<td>".$satir["ya"]."</td>");
    echo("<td>".$satir["ys"]."</td>");
    echo("<td>".$satir["yu"]."</td>");
    echo("<td><a href='guncellenecek.php?yazarkayitno=".$satir["yazarkayitno"]."'>Güncelle</a></td>");
    echo("<td><a href='sil.php?yazarkayitno=".$satir["yazarkayitno"]."'>Sil</a></td>");
    echo("</tr>");
            }
echo("</table>");
?>
